...
try:
    ...
    print(
        "ERROR: Required packages not found. Run: pip install pandas numpy matplotlib scikit-learn"
    )
    sys.exit(1)
...
def train_ml_model(df):
    ...
    # Pro features: version, tcp_flags (if available)
...
def compare_classifiers(csv_path, output_dir="ml/results"):
    ...
    kernel_metrics = compute_metrics(
        test_df["label_norm"], test_df["kernel_label_norm"], "Kernel (Rule-based)"
    )

    ml_metrics = compute_metrics(
        test_df["label_norm"], test_df["ml_label_norm"], "User-space (ML)"
    )
    ...
    print("-" * 52)
    print(
        f"{'Accuracy':<20} {kernel_metrics['accuracy']:>14.2%} {ml_metrics['accuracy']:>14.2%}"
    )
    print(
        f"{'Precision':<20} {kernel_metrics['precision']:>14.2%} {ml_metrics['precision']:>14.2%}"
    )
    print(
        f"{'Recall':<20} {kernel_metrics['recall']:>14.2%} {ml_metrics['recall']:>14.2%}"
    )
    print(f"{'F1-Score':<20} {kernel_metrics['f1']:>14.2%} {ml_metrics['f1']:>14.2%}")
    print(
        f"{'Total Samples':<20} {kernel_metrics['total']:>15} {ml_metrics['total']:>15}"
    )
    ...
    print(
        classification_report(
            df["label_norm"], df["kernel_label_norm"], zero_division=0
        )
    )
    ...
    cm_ml = confusion_matrix(
        df["label_norm"], df["ml_label_norm"], labels=sorted(df["label_norm"].unique())
    )
    ConfusionMatrixDisplay(cm_ml, display_labels=labels).plot(
        ax=axes[1], colorbar=False, cmap="Greens"
    )
    axes[1].set_title(
        "User-space Classifier\n(ML - Random Forest)", fontsize=12, fontweight="bold"
    )
    axes[1].tick_params(axis="x", rotation=45)

    metrics = ["Accuracy", "Precision", "Recall", "F1-Score"]
    kernel_vals = [
        kernel_metrics["accuracy"],
        kernel_metrics["precision"],
        kernel_metrics["recall"],
        kernel_metrics["f1"],
    ]
    ml_vals = [
        ml_metrics["accuracy"],
        ml_metrics["precision"],
        ml_metrics["recall"],
        ml_metrics["f1"],
    ]
    ...
    axes[2].bar(
        x - width / 2, kernel_vals, width, label="Kernel", color="steelblue", alpha=0.8
    )
    axes[2].bar(
        x + width / 2,
        ml_vals,
    ...
    results = {"kernel": kernel_metrics, "ml": ml_metrics}
    ...
    print(
        f"Accuracy: Kernel={kernel_metrics['accuracy']:.0%}, ML={ml_metrics['accuracy']:.0%}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare kernel vs user-space classifiers"
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    parser.add_argument(
        "--output", "-o", default="ml/results", help="Output directory for plots"
    )
