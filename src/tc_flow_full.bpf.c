...
// Uses perf ring buffer for real-time events + hash map for state.
...
int tc_flow(struct __sk_buff *skb) {
    ...
    if (ip->protocol == IPPROTO_TCP || ip->protocol == IPPROTO_UDP || ip->protocol == IPPROTO_SCTP) {
        void *transport = (void *)ip + ihl;
        parse_tcp_udp_ports(transport, data_end, &key.src_port, &key.dst_port);
