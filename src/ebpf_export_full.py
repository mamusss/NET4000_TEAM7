...
"""
ebpf_export_full.py
Pro-grade flow exporter with IPv4/IPv6 support, TCP flags, and Threat detection.
"""
...
def threat_label_name(val):
    return THREAT_LABELS.get(val, "NONE")
...
def parse_flows_from_map(raw_entries):
    ...
    for entry in raw_entries:
        try:
            ...
            # key: src_ip[4], dst_ip[4], proto(1), ver(1), src_port(2), dst_port(2) = 38 bytes
            ...
            # value: pkt_count(8), byte_count(8), first_ts(8), last_ts(8), ipt_sum(8), min_ipt(8), max_ipt(8), tcp_flags(1), label(1), threat(1), pad(5) = 64 bytes
            pkts = le64(val_raw, 0)
            ...
            flows.append(
                FlowRecord(
                    ...
                    threat_level=threat_label_name(t_level),
